...
- [/] 3.3. Implement EF Core Configurations in the Infrastructure layer. — Part 1: Org/HR entities (Company, CompanyBranch, Department, Employee, Position). Part 2: Implemented Business Partner entities (BusinessPartner, BusinessPartnerCategory, BusinessPartnerSite, BusinessPartnerContact). All FKs use `OnDelete(Restrict)`, enums stored as strings, composite unique indexes, max length constraints. Remaining: ProductService group (7 entities).
